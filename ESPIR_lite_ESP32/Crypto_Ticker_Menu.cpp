...
Crypto_Ticker_Menu::Crypto_Ticker_Menu(Adafruit_GFX* tft, COIN** coins, Portfolio* portfolio, Network_Manager_ESP32* network_manager)
  ...
  // load menu from storage if present, otherwise reset it to default values and save
  if (!menu->load()){
    menu->reset();
  }
}
...
void Crypto_Ticker_Menu::displayMemoryInfo() {
    uint8_t start = tft->height() - 30;
    tft->fillRect(0, start, tft->width(), tft->height() - start, BLACK);
    
    // Get memory information
    multi_heap_info_t heapInfo;
    heap_caps_get_info(&heapInfo, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    
    // Calculate fragmentation percentage
    uint32_t totalBytes = heap_caps_get_total_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    uint32_t freeBytes = heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    uint32_t largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    int fragmentation = 100 - (largestBlock * 100) / freeBytes;
    ...
    // Color code fragmentation percentage
    if (fragmentation < 33)
        tft->setTextColor(LIGHT_GREEN);
    else if (fragmentation < 66)
