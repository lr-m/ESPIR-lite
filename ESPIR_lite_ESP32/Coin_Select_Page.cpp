...
bool Coin_Select_Page::pageSerialize(uint8_t *buffer, int *index, uint16_t length_limit) {
    ...
    uint8_t value = 0;
    int bytes_needed = (COIN_COUNT + 7) / 8; // Calculate needed bytes, rounding up

    // Check if we have enough space in buffer
    if (*index + bytes_needed > length_limit) {
        return false;
    }
    ...
    for (int i = 0; i < COIN_COUNT; i++) {
        ...
        if (i == COIN_COUNT - 1) {
            if (COIN_COUNT % 8 != 0) {  // Only pad if not perfectly aligned
                int padding_bits = 8 - (COIN_COUNT % 8);
